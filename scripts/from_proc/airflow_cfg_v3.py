...
def get_group_flag(group_no, if_ck_date=True, if_ck_depending=True, tx_date=(date.today() + timedelta(days=-1)).strftime('%Y%m%d')):
    sql = '''
    select db_type
           ,db_ip
           ,db_user
    ...
    # check task depending
    if not if_ck_depending:
        return '1'
